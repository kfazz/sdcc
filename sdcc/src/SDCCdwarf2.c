...
{
  //length
  dwWriteWord(NULL, dwSizeofCFAinstructions(fp->ins) + 4 + port->debugger.dwarf.addressSize * 2, NULL);
...
dwHashType(sym_link *type)
{
  ...
  while (type)
  {
    ...
    {
      hash ^= SPEC_NOUN(type) | (SPEC_CONST(type) << 4) | (SPEC_VOLATILE(type) << 5) | (SPEC_LONG(type) << 6) | (SPEC_LONGLONG(type) << 7);
    }
...
/*-----------------------------------------------------------------------*/
int dwWriteLabel(symbol *sym, const iCode *ic)
{
  char debugSym[SDCC_NAME_MAX + 1];
  dwtag *tp;
